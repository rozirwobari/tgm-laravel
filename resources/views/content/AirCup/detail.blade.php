...
@section('content')
    <div class="rzw-container" style="display: flex;">
        ...
        <div class="rzw-box-content" style="flex: 0 0 85%; padding: 1px; border-radius: 0 10px 10px 0;">
            <h5 class="pt-2" style="font-weight: 600;">QC Air CUP</h5>
        </div>
    </div>
    ...
    <form action="{{ url('/qc_air_cup/edit/' . $details->id) }}" method="POST">
        @csrf
        <div class="rzw-box-content">
            <div class="card-body">
                ...
                @foreach (RZWHelper::FilterName($decode_qc, 'tds_input_') as $key => $item)
                    <div class="input-group my-3">
                        <input type="text" class="form-control rzw-input" name="{{ $key }}"
                            ...
                            value="{{ $item }}"
                            style="background-color: {{ $item == null ? 'white' : ($item >= 0 && $item <= 5 ? 'green' : ($item > 6 && $item <= 10 ? '#ecd700' : '#ff0000')) }}; color: {{ $item == null ? 'black' : ($item >= 0 && $item <= 5 ? 'white' : ($item > 6 && $item <= 10 ? 'black' : 'white')) }};"
                            {{ $details->status == 0 && $details->user_id == Auth::user()->id && Auth::user()->role->name != 'viewers' ? '' : 'readonly' }}>
                        <div class="input-group-prepend">
                            <span class="rzw-icon-input" style="z-index: 5;">
                ...
                @foreach (RZWHelper::FilterName($decode_qc, 'ph_input_') as $key => $item)
                    <div class="input-group my-3">
                        <input type="text" class="form-control rzw-input" name="{{ $key }}"
                            id="{{ $key }}" placeholder="Value {{ $i++ }}"
                            value="{{ $item }}"
                            style="background-color: {{ $item == null ? 'white' : ($item >= 5.0 && $item <= 7.0 ? 'green' : ($item > 7.1 && $item <= 7.5 ? '#ecd700' : '#ff0000')) }}; color: {{ $item == null ? 'black' : ($item >= 5.0 && $item <= 7.0 ? 'white' : ($item > 7.1 && $item <= 7.5 ? 'black' : 'white')) }};"
                            {{ $details->status == 0 && $details->user_id == Auth::user()->id && Auth::user()->role->name != 'viewers' ? '' : 'readonly' }}>
                        <div class="input-group-prepend">
                            <span class="rzw-icon-input" style="z-index: 5;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
    ...
    @if ($details->status == 0 && Auth::user()->role->name == 'manager')
        <div class="rzw-box-content">
            <div class="row">
                <div class="col-6">
                    <a class="btn btn-primary w-100" href="{{ url('qc_air_cup/reject/' . $details->id) }}"
            ...
            <div class="row mt-3">
                <div class="col-12">
                    <a class="btn btn-primary w-100" href="{{ url('qc_air_cup/approve/' . $details->id) }}"
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{ url('qc_air_cup/delete') }}" + "/" + id;
                }
            });
        }
    </script>
