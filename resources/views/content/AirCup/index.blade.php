...
@section('content')
    ...
    <div class="mt-3 overflow-auto" style="{{ Auth::user()->name != 'viewers' ? 'max-height: 70vh;' : 'max-height: 86vh;'; }} scrollbar-width: none;">
        @foreach ($data as $value)
            <div class="rzw-box-content text-start">
                <a href="{{ url('/qc_air_cup/detail/' . $value->id) }}">
                    <div class="card-body">
                        <p class="fw-bold">{{ RZWHelper::formatTanggalIndonesia($value->created_at) }} [<span style="color: {{ $value->status == 0 ? '#c6a200' : ($value->status == 1 ? 'green' : 'red') }}">{{ $value->status == 0 ? 'Pending' : ($value->status == 1 ? 'Approve' : 'Reject') }}</span>]</p>
